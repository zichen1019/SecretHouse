<template>
    <div style="width:95%;margin: 5px auto;">
        <el-card class="box-card" style="width: 100%;">
            ...
            <el-table ref="todayMustList" :data='todayMustList' tooltip-effect="dark" style="width: 100%" 
                @selection-change="handleSelectionChange" max-height="310">
                <el-table-column type="selection" width="55" align="center" fixed></el-table-column>
                <el-table-column label="序号" type="index" width="50" align="center" fixed></el-table-column>
                <el-table-column label="创建日期" width="120" align="center" fixed>
                    <template slot-scope="scope">
                        {{ scope.row.createDate.substring(0,10) }}</template>
                </el-table-column>
                <el-table-column label="执行时间" width="120" align="center" fixed>
                        <template slot-scope="scope">
                            <!-- 此处的表尽可能的根据时间的越来越近，并且任务待办的情况下，用颜色来表示轻重缓急 -->
                            <i class="el-icon-time"></i> 
                            {{ scope.row.executionTime.substring(12) }}</template>
                    </el-table-column>
                <el-table-column prop="title" label="必达主题" width="470" align="center" show-overflow-tooltip>
                    <!--  如果是单条则是修改  -->
                        <template slot-scope="scope">
...
<script>
    ...
    import { getTodayMustList,deleteTodaysMust } from '@/api/todaysMust' // 请求
    ...
    export default {
        data(){
            return {
                ...
                listQuery: {
                    ...
                    today: true,// 是否是当天
                    title: undefined, //主题
                    complete: undefined // 是否已完成
                },
                // 状态选框
                completeList: [{label:'未完成',key:false},{label:'已完成',key:true}],
                // 今日必达事项
                todayMustList: [{id:'1',zj:'1',createDate:'21:04:00',executionTime:'22:00:00',title: '01',todaymust:'todaymust1',complete:true,sort:1}],
                // 当前选中的数据
                multipleSelection: {},
                // 是否正在下载文件
        ...
        methods: {
            ...
            getList(){
                 ...
                 * 获取今日必达事项
                 */
                this.listQuery.zj = this.user
                getTodayMustList(this.listQuery).then(response => {
                    console.log('查询今日必达完毕');
                    console.log(response)
                    console.log(response.data.todaysmust)
                    this.todayMustList = response.data.todaysmust
                })
            },
            batchDelete() {
                /**
